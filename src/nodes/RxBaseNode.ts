...
export abstract class RxBaseNode<TConfig extends NodeConfig = NodeConfig> {
   ...
   */
  protected abstract createProcessingOperator(): (
   ...
   */
  protected handlePacketError(
    error: Error, 
    caught: Observable<DataPacket>
  ): Observable<DataPacket> {
    ...
    switch (this.getBufferStrategy()) {
      case 'time':
        return (source) => source.pipe(
          bufferTime(this.getBufferDuration(), undefined, bufferSize)
        );
      
      case 'count':
        return (source) => source.pipe(
          bufferCount(this.getBufferSize(), undefined)
        );
      
      default:
        return (source) => source.pipe(
          buffer(this.getBufferTrigger$())
        );
    }
  }
