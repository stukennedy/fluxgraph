...
export class GraphRunner {
  ...
  private eventListeners: Map<GraphEventType, Set<(event: GraphEvent) => void>> = new Map();
  private subscriptions: Map<string, GraphSubscription> = new Map();
  private executionTimer?: number;
   ...
   */
  private createNode(config: AnyNodeConfig): BaseNode {
    switch (config.type) {
      case 'source':
   ...
   * Evaluate condition
   */
  private async evaluateCondition(condition: string, packet: DataPacket): Promise<boolean> {
    try {
   ...
   */
  private async applyTransformation(transform: string, packet: DataPacket): Promise<DataPacket> {
    try {
   ...
   * Handle node error
   */
  private handleNodeError(nodeId: string, error: Error, packet?: DataPacket): void {
