...
import { interval, merge, combineLatest, of, timer } from 'rxjs';
import {
  map,
  filter,
  scan,
...
export function backpressureExample() {
  // Fast producer
  const fastProducer$ = interval(10).pipe(map((i) => ({ id: i, data: `Fast item ${i}` })));
...
if (typeof window === 'undefined') {
  ...
  financialProcessingExample()
    .pipe(take(5))
    .subscribe((result) => console.log('Financial:', result));

  // Example 2: Analytics
  analyticsExample()
    .pipe(take(5))
    .subscribe((result) => console.log('Analytics:', result));
